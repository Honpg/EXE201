...
def generate_overall_summary(transcript_data: list) -> str:
    ...
    full_transcript = " ".join([
        ...
        for entry in transcript_data if is_meaningful(entry['content'])
    ])
    return summarize_with_gemini(full_transcript, "Provide a concise executive summary of this meeting transcript.")
...
def generate_key_takeaways(transcript_data: list) -> str:
    ...
    return summarize_with_gemini(full_transcript, "Extract the key takeaways and action items from this meeting transcript. Present them as a clear, scannable list.")
...
def generate_speaker_summaries(transcript_data: list, speaker_durations: dict) -> dict:
    ...
    for speaker, points in speaker_contributions.items():
        ...
        summary = summarize_with_gemini(combined_points, f"Summarize the key points made by {speaker}.")
...
def create_normal_report_pdf(meeting_data):
    ...
    elements = [Paragraph(f"<b>{meeting_data['meetingTitle']}</b>", styles['Title']), Spacer(1, 12)]
    ...
    elements.append(Paragraph(generate_overall_summary(meeting_data['transcriptData']), styles['BodyText']))
    elements.append(Spacer(1, 12))
    
    elements.append(Paragraph("Key Takeaways", styles['h2']))
    elements.append(Paragraph(generate_key_takeaways(meeting_data['transcriptData']), styles['BodyText']))
    elements.append(Spacer(1, 12))

    elements.append(Paragraph("Speaker Summaries", styles['h2']))
    speaker_summaries = generate_speaker_summaries(meeting_data['transcriptData'], meeting_data['speakerDuration'])
    for speaker, data in speaker_summaries.items():
        elements.append(Paragraph(f"<b>{speaker}</b> ({data['duration']} seconds)", styles['h3']))
...
def create_normal_report_docx(meeting_data):
    ...
    doc.add_paragraph(f"Time: {format_time(meeting_data['meetingStartTimeStamp'])} - {format_time(meeting_data['meetingEndTimeStamp'])}")
...
def create_sentiment_report_pdf(meeting_data):
    ...
    elements = [Paragraph("Sentiment Analysis Report", styles['Title']), Spacer(1, 12), Image(chart_filename, width=4*inch, height=4*inch), Spacer(1, 12)]

    for entry in analysis:
        elements.append(Paragraph(f"<b>{entry['speaker']}</b> ({entry['sentiment_category']})", styles['Normal']))
...
def generate_reports(meeting_data, report_type="Normal", format_type="PDF", interval_minutes=5):
    """
    ...
    if func:
        ...
        raise ValueError(f"Invalid report/format combination: {report_type}/{format_type}")
...
if __name__ == "__main__":
    ...
    sample_meeting_data = {
        ...
        'transcriptData': [
            ...
            {'name': 'Prateek', 'content': 'Let\'s discuss about the features that our extension is going to give to the users. The first is that it will give transcriptions, which it will scrape from Google Meet captions.', 'timeStamp': '2024-09-29T12:21:45.000Z'},
            {'name': 'TitaNyte Official', 'content': 'Sure. Once those transcripts are stored in the database, we have a generative AI model which takes these transcripts and then tries to create reports based on filters like speaker-based or interval-based reporting.', 'timeStamp': '2024-09-29T12:22:12.000Z'},
            {'name': 'TitaNyte Official', 'content': 'On the front end, you log in using Google. As soon as you click the Generate button, a modal will pop up where you can enter the meeting title and choose the report format, like DOCX or PDF.', 'timeStamp': '2024-09-29T12:23:02.000Z'},
            {'name': 'TitaNyte Official', 'content': 'You can also add Prompts to enhance the report creation. The extension also lets you take screenshots to keep track of what was shared during the meeting.', 'timeStamp': '2024-09-29T12:23:42.000Z'},
            {'name': 'Prateek', 'content': 'Okay, so now I will be covering how anyone can use our extension through our repo. First, they\'ll have to clone the repository and install all the packages. It is already available on the Chrome store.', 'timeStamp': '2024-09-29T12:24:30.000Z'},
            {'name': 'Prateek', 'content': 'They will have to do an NPM install for both front-end and back-end, and a pip install for the AI back-end. Afterwards, they will have to set the environment variables, which we will provide.', 'timeStamp': '2024-09-29T12:25:02.000Z'},
            {'name': 'TitaNyte Official', 'content': 'You also have to add a .env.example file, which shows the format for the environment variables. I think we\'re good to go, right?', 'timeStamp': '2024-09-29T12:25:42.000Z'},
            {'name': 'Prateek', 'content': 'Yeah, we are good to go. Thank you.', 'timeStamp': '2024-09-29T12:26:02.000Z'}
        ]
    }
