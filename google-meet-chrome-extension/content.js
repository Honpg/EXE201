...
function meetingRoutines(uiType) {
  ...
  checkElement(meetingEndIconData.selector, meetingEndIconData.text).then(() => {
    ...
    try {
      ...
      // ✅ MEETING END
      contains(meetingEndIconData.selector, meetingEndIconData.text)[0]
...
//       const transcriptRoot = document.querySelector('.a4cQT');
// if (transcriptRoot?.firstChild?.firstChild?.childNodes?.length > 0) {
//   const people = transcriptRoot.firstChild.firstChild.childNodes;
//   // xử lý như bình thường
// } else {
//   console.warn("Transcript DOM not found or has changed.");
//   return;
// }
...
// function transcriber(mutationsList, observer) {
//   try {
//     const transcriptRoot = document.querySelector('.a4cQT .VbkSUe');
//     if (!transcriptRoot) {
//       console.warn("Transcript DOM not found.");
//       return;
//     }

//     const captionLines = Array.from(transcriptRoot.querySelectorAll('div'))
...
//       personName: "You", // Hoặc lấy tên thực từ biến nếu bạn có
//       timeStamp: timeStamp,
...
//   } catch (error) {
//     console.error("❌ Lỗi transcriber:", error);
...
function transcriber(mutationsList, observer) {
  try {
    const peopleNodes = document.querySelectorAll('.bj4p3b'); // mỗi người nói
    if (!peopleNodes || peopleNodes.length === 0) {
      console.warn("Transcript DOM not found or empty.");
      return;
    }

    peopleNodes.forEach(personNode => {
      const nameNode = personNode.querySelector('.NWpY1d');
      const transcriptNode = personNode.querySelector('.ygicle.VbkSUe');

      if (nameNode && transcriptNode) {
        const name = nameNode.textContent.trim();
        const text = transcriptNode.textContent.trim();

        if (text && text !== transcriptTextBuffer) {
          const timeStamp = new Date().toLocaleString("default", timeFormat).toUpperCase();
...
//   try {
//     const captionBlock = document.querySelector('.a4cQT .VbkSUe');
//     if (!captionBlock) {
//       console.warn("Transcript DOM not found.");
//       return;
//     }

//     const text = captionBlock.textContent.trim();
//     if (!text) return;
...
// function transcriber() {
//   try {
//     const peopleNodes = document.querySelectorAll('.bj4p3b'); // vùng người nói
//     if (!peopleNodes || peopleNodes.length === 0) {
//       console.warn("Transcript DOM not found or empty.");
//       return;
//     }
...
//     peopleNodes.forEach(personNode => {
//       const nameNode = personNode.querySelector('.zs7s8d');
//       const transcriptNode = personNode.querySelector('.VbkSUe');

//       if (nameNode && transcriptNode) {
//         const name = nameNode.textContent.trim();
//         const text = transcriptNode.textContent.trim();

//         // Chỉ lưu nếu caption khác cái trước
//         if (text && text !== transcriptTextBuffer) {
//           const timeStamp = new Date().toLocaleString("default", timeFormat).toUpperCase();
...
// function transcriber() {
//   try {
//     const peopleNodes = document.querySelectorAll('.a4cQT .bj4p3b'); // Danh sách người đang nói
//     if (!peopleNodes || peopleNodes.length === 0) {
//       console.warn("Transcript DOM not found or empty.");
//       return;
//     }
...
//     peopleNodes.forEach(personNode => {
//       const nameNode = personNode.querySelector('.zs7s8d');       // Tên người nói
//       const transcriptNode = personNode.querySelector('.VbkSUe'); // Caption
//       if (nameNode && transcriptNode) {
//         const name = nameNode.textContent.trim();
//         const text = transcriptNode.textContent.trim();

//         if (text && text !== transcriptTextBuffer) { // Chỉ lưu nếu khác caption trước
//           const timeStamp = new Date().toLocaleString("default", timeFormat).toUpperCase();
...
function overWriteChromeStorage(keys, endMeeting) {
  ...
  // // ❗ Thêm đoạn lưu vào chrome storage
  // chrome.storage.local.set(objectToSave, () => {
  //   if (chrome.runtime.lastError) {
  //     console.error("❌ Error saving to chrome.storage:", chrome.runtime.lastError);
  //   } else {
  //     console.log("✅ Saved to chrome.storage:", objectToSave);

  //     // Nếu cần làm gì đó khi kết thúc họp
  //     if (endMeeting) {
